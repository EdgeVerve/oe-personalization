...
function findByIdAndCreate(self, q, data, options, cb) {
  self.find(q, options, function (err, r) {
    ...
    if (r && r.length >= 1) {
      ...
      if (!_.isEqual(options.ctx, resultScope)) {
...
function callUpdateOnModel(self, actualFn, args) {
  ...
  if (!model.definition.settings.mixins.DataPersonalizationMixin) {
    return actualFn.apply(self, ary);
  }
  var id;
  if (!args.id) {
    id = util.getIdValue(model, args.data);
  } else {
    id = args.id;
  }
  ...
  findByIdAndCreate(model, q, args.data, newCtx, function (err, r) {
    if (err === 'NO-ACTION') {
      ary[optionsIndex - 1] = newCtx;
      return actualFn.apply(self, ary);
    }
    if (err) {
      return args.cb(err);
...
DataAccessObject.replaceOrCreate = DataAccessObject.updateOrCreate = function (data, options, cb) {
  ...
  return callUpdateOnModel(self, function (data, opts, cb) {
    return _replaceById.call(self, id, data, opts, cb);
  },
  { data, options, cb });
};
